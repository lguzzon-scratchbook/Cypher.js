...
var GraphViewer = function (options) {
	...
	var preprocessGraphData = (nodes, links) => {
		var nodeIds = {};
	...
	this.label = (index) => labels[index];

	this.labelCount = (index) => labelInfo[labels[index]].count;
...
GraphViewer.prototype.draw = function () {
	...
	var legend = svg.append('g').attr('transform', 'translate(5, 5)');
	...
	legendEntry
		...
		.attr('x', 7)
		.attr('y', 6 / 2)
		.text((d) => this.label(d - 1) + ' (' + this.labelCount(d - 1) + ')')
		.attr('class', 'legend')
		.on('mouseover', (d) => {
			d3.selectAll('.__' + this.label(d - 1)).attr('r', radius + 2);
		})
		.on('mouseout', (d) => {
			d3.selectAll('.__' + this.label(d - 1)).attr('r', radius);
		});
	...
	var line = link
		.append('line')
		.attr('marker-end', (d) =>
			d.direction == 'right' || d.direction == 'both' ? 'url(#arrow)' : null
		)
		.attr('marker-start', (d) =>
			d.direction == 'left' || d.direction == 'both' ? 'url(#arrow)' : null
		);

	line.append('title').text((d) => JSON.stringify(d));
	...
	node
		...
		.attr('class', (d) => '__' + d.labels[0] || 'No Label')
		.append('title')
	...
	function ticked() {
		...
		linkText
			.attr(
				'x',
				(d) => d.source.x + (d.target.x - d.source.x) / 2 + (radius / 2 - 0.5)
			)
			.attr(
				'y',
				(d) => d.source.y + (d.target.y - d.source.y) / 2 + (radius / 2 - 0.5)
			)
			.attr('transform', function (d) {
				...
				fitText(Math.sqrt(deltaX ** 2 + deltaY ** 2) - 30, this, d.type);
